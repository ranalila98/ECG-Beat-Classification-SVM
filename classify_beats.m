...
function [n_abnorm, n_beats_det, y_pred, R_loc, P_loc, Q_loc, S_loc, T_loc] = classify_beats(ecg, SVMModel, template_normal)
% CLASSIFY_BEATS Detects and classifies ECG beats as normal/abnormal.
% Inputs:
%   ecg             - Raw ECG signal (1D array)
%   SVMModel        - Trained SVM model
%   template_normal - (Optional) Template vector for correlation (1D)
% Outputs:
%   n_abnorm        - Number of abnormal beats
%   n_beats_det     - Total detected beats
%   y_pred          - Predicted labels for each beat (1=normal, 0=abnormal)
%   R_loc, P_loc, ... - Peak locations in ECG
...
%% Feature Extraction
% Make template_normal optional
if nargin < 3
    template_normal = [];  % Disable correlation if not provided
end

[feature_vector, R_loc, Q_loc, S_loc, P_loc, T_loc, n_beats_det, ~, ~] = ...
    extract_ecg_features(ecg_m, ecg_MW, fs, template_normal);
